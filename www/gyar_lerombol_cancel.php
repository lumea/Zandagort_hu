...
$cron_er=mysql_query('select *,coalesce(100*(1-timestampdiff(second,now(),mikor_aktualis)/timestampdiff(second,mikor_kiadva,mikor_aktualis)),-1) as keszenlet from cron_tabla where id='.$_REQUEST['cron_id'].' and feladat='.FELADAT_GYAR_LEROMBOL);
$cron=mysql_fetch_array($cron_er);
if (!$cron) kilep($lang[$lang_lang]['kisphpk']['Nincs ilyen dózerolás.']);
...
mysql_query('insert into cron_tabla (mikor_aktualis,feladat,bolygo_id,gyar_id,aktiv,darab,mikor_kiadva,indulo_allapot) values("'.date('Y-m-d H:i:s',round(time()+(100-$szazalek)/100*60*$aux4['ido'])).'",'.FELADAT_GYAR_EPIT.','.$cron['bolygo_id'].','.$cron['gyar_id'].','.$cron['aktiv'].','.$cron['darab'].',"'.date('Y-m-d H:i:s',round(time()-$szazalek/100*60*$aux4['ido'])).'",'.$cron['indulo_allapot'].')')or hiba(__FILE__,__LINE__,mysql_error());
