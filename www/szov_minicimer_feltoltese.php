...
function php_kilep($s) {
	kilep('<html><body onload="window.top.window.inline_toggle(\'uj_minicimer_tolto\');alert(\''.$s.'\');"></body></html>');
}
...
if (is_uploaded_file($_FILES['kep']['tmp_name'])) {
	...
	if ($orig_meret) {//eredeti meret
		move_uploaded_file($_FILES['kep']['tmp_name'],'img/minicimerek/p'.$szovetseg['id'].$szovetseg['minicimer_crc'].'.'.$ext);
	} else {//atmeret
		$tn=imagecreatetruecolor($tw,$th);
		imagecopyresampled($tn,$kep,0,0,0,0,$tw,$th,$w,$h);
		$cel_fajl='img/minicimerek/p'.$szovetseg['id'].$szovetseg['minicimer_crc'].'.'.$ext;
		switch($ext) {
			case 'jpg':
	...
	mysql_query('update szovetsegek set minicimer_ext="'.$ext.'",kepfajl_random="'.randomgen(32).'" where id='.$szovetseg['id']);
